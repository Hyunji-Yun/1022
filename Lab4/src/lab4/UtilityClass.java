...
public class UtilityClass {
	...
	public static String sumofEvensandOdds(int lowerLimit, int upperLimit) {
		...
		return nums+ " numbers between " + lowerLimit+" and "+ upperLimit +": sum of evens ("+even+") and sum of odds ("+odd+")";
	}
	...
	public static String binaryValue(short value) {
		...
		String ans = String.format("%16s", binary).replaceAll(" ", "0");
		
		return "The integer value ("+ value +") has 16 bits binary representation ["+ ans +"]";
	}
	...
	public static String countofMult35(int lowerLimit, int upperLimit) {
		...
		for(int i=lowerLimit; i<=upperLimit; i++){
			...
			if(i%3==0) {
		 	...
		 	return "Between "+lowerLimit+" and "+ upperLimit+" there are ("+three+") multiple of 3 and there are ("+five+") multiple of 5";
	}
	...
	public static int getRandomNumber(int x, int y) {
		...
		/* Your implementation here. */
		int random = (int)Math.floor(Math.random()*(y-x+1)+x);
